			...
			<div className="flex justify-center">
				...
				>
					...
					<p className="mt-1 text-[11px] leading-relaxed text-zinc-500 dark:text-zinc-400">
						{t("graph.mindMapSubtitle")}
					</p>
					<h2 className="mt-3 text-xl font-semibold leading-snug text-zinc-900 dark:text-zinc-100">
						{topic}
					</h2>
					{kws.length > 0 && (
						<div className="mt-4 flex flex-wrap items-center justify-center gap-2">
							<span className="text-[11px] font-medium text-zinc-500 dark:text-zinc-400">
								{t("graph.themeKeywords")}
							</span>
							{kws.map((kw) => (
								<span
									key={kw}
			...
			<ul className="flex flex-1 flex-col gap-2 p-3 text-sm">
				{category.entries?.length ? (
					category.entries.map((entry) => (
						<MindMapEntryBlock key={entry.id} entry={entry} accent={color} />
					))
				) : (
...
function MindMapEntryBlock(props: {
	entry: MindMapEntry;
	accent: string;
}): JSX.Element {
	...
	return (
		<li className="rounded-lg border border-zinc-200/90 bg-zinc-50/80 px-2.5 py-2 dark:border-zinc-700 dark:bg-zinc-800/60">
			...
			{entry.children && entry.children.length > 0 && (
				<ul
					className="mt-2 space-y-1 border-l-2 pl-2 text-xs text-zinc-600 dark:text-zinc-400"
...
export function GraphViewer(): JSX.Element {
	...
	const [mindMap, setMindMap] = useState<GlobalMindMapFile | null | undefined>(
		undefined,
	);
	...
	const showMindMapPanel =
		hasMindMap && (viewMode === "mindmap" || !hasNetwork);
	const showNetworkPanel =
		hasNetwork && (viewMode === "network" || !hasMindMap);
