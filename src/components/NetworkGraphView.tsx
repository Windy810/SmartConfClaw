...
function FitViewOnLayoutChange(props: { layoutSig: string; refreshKey: number }): null {
...
export function NetworkGraphView(props: NetworkGraphViewProps): JSX.Element {
  ...
  const positions = useMemo(() => layoutKnowledgeGraph(nodes, edges), [nodes, edges]);
  ...
  return (
    <div className="flex h-[680px] w-full min-h-0">
      <ReactFlowProvider>
        ...
        >
          <FitViewOnLayoutChange layoutSig={layoutSig} refreshKey={refreshKey} />
          <MiniMap
            pannable
            zoomable
            nodeStrokeWidth={2}
      ...
      {selectedNode ? (
        <aside className="flex w-[min(100%,320px)] shrink-0 flex-col border-l border-zinc-200 bg-zinc-50/95 dark:border-zinc-800 dark:bg-zinc-900/95">
            <div className="flex items-start justify-between gap-2 border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: nodeAccent(selectedNode.group) }}>
                  {selectedNode.group}
                </p>
                <h3 className="mt-1 text-base font-semibold leading-snug text-zinc-900 dark:text-zinc-50">{selectedNode.label}</h3>
              </div>
              <Button type="button" variant="ghost" size="sm" className="h-8 shrink-0 text-xs" onClick={() => setSelectedId(null)}>
                {t("graph.closePanel")}
              </Button>
            </div>
            <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4 text-sm">
              {selectedNode.summary ? (
                <section>
                  <h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-zinc-500">{t("graph.nodeSummary")}</h4>
                  <p className="leading-relaxed text-zinc-700 dark:text-zinc-300">{selectedNode.summary}</p>
                </section>
              ) : (
                <p className="text-xs text-zinc-500">{t("graph.noNodeSummary")}</p>
              )}
              {selectedNode.keyPoints && selectedNode.keyPoints.length > 0 ? (
                <section>
                  <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500">{t("graph.keyPoints")}</h4>
              ...
              {selectedNode.sessions && selectedNode.sessions.length > 0 ? (
                <section>
                  <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500">{t("graph.sourceSessions")}</h4>
