...
function KnowledgeGraphNodeInner(props: NodeProps<KnowledgeGraphNodeData>): JSX.Element {
  const { data } = props;
  ...
  return (
    <div
      className={`max-w-[260px] cursor-pointer rounded-xl border-2 bg-white px-3 py-2.5 shadow-sm transition-[box-shadow,transform] dark:bg-zinc-900 ${
        selected ? "ring-2 ring-offset-2 ring-violet-500 ring-offset-zinc-100 dark:ring-offset-zinc-950" : "hover:shadow-md"
      }`}
    ...
    >
      ...
      {graphNode.summary ? (
        <p className="mt-1 line-clamp-2 text-[11px] leading-relaxed text-zinc-600 dark:text-zinc-400">{graphNode.summary}</p>
      ) : null}
      <p className="mt-1.5 text-[10px] text-zinc-400 dark:text-zinc-500">{clickHint}</p>
